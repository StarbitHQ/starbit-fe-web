...
type Action = "email" | "account_bal" | "suspend" | "promote" | "delete" | null; // ← Added "delete"
...
const account_balSchema = z.object({
  account_bal: z.coerce
    .number()
    .refine(
      (v) => !v.toString().includes(".") || v.toString().split(".")[1].length <= 2,
...
export const QuickActions = ({
    ...
    if (active === "promote" && roles.length === 0) {
      const fetchRoles = async () => {
        ...
        try {
          ...
          let rolesData;
          if (response.data?.roles) rolesData = response.data.roles;
          else if (response.roles) rolesData = response.roles;
          else if (Array.isArray(response.data)) rolesData = response.data;
          else if (Array.isArray(response)) rolesData = response;
  ...
  const handleaccount_bal = async (data: z.infer<typeof account_balSchema>) => {
    ...
    try {
      await api.patch(`/api/admin/users/${userId}/balance`, {
        account_bal: parseFloat(data.account_bal.toFixed(2)),
      });
      setResult("success");
  ...
  const handleSuspend = async () => {
    ...
    try {
      ...
      setResult("error");
      toast({ title: "Error", description: e.message ?? "Failed to suspend user", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePromote = async (data: z.infer<typeof promoteSchema>) => {
    ...
    setResult(null);
    try {
      await api.post(`/api/admin/${userId}/promote`, { role: data.role });
      setResult("success");
      ...
      toast({
        title: "Error",
        description: e.response?.data?.error || "Failed to update role",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onUserUpdate?.(); // Let parent refresh or redirect
      setTimeout(() => setActive(null), 1500);
    } catch (e: any) {
      setResult("error");
      toast({
    ...
    <Card className="bg-muted/30 border-border">
      ...
      <CardContent className="space-y-8">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            variant="outline"
            className="flex-1 min-w-[140px] justify-center gap-2 text-red-600 hover:text-red-700 border-red-300 hover:border-red-400"
            ...
            <form onSubmit={emailForm.handleSubmit(handleEmail)} className="space-y-3">
              <div>
                ...
                {emailForm.formState.errors.subject && (
                  <p className="text-sm text-red-500 mt-1">{emailForm.formState.errors.subject.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {emailForm.formState.errors.message && (
                  <p className="text-sm text-red-500 mt-1">{emailForm.formState.errors.message.message}</p>
                )}
              </div>
        ...
        {active === "account_bal" && (
          <div className="rounded-md border bg-background p-4 space-y-3">
            ...
            <form onSubmit={account_balForm.handleSubmit(handleaccount_bal)} className="space-y-3">
              <input
                type="number"
                step="0.01"
              ...
              {account_balForm.formState.errors.account_bal && (
                <p className="text-sm text-red-500">{account_balForm.formState.errors.account_bal.message}</p>
              )}
          ...
          <div className="rounded-md border bg-background p-4 space-y-3">
            ...
            <form onSubmit={promoteForm.handleSubmit(handlePromote)} className="space-y-3">
              {fetchingRoles ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Debug: isSuperAdmin={String(isSuperAdmin)}, roles.length={roles.length}, userRole={userRole}
                  </p>
                </div>
              ) : (
                <>
                  ...
                  {promoteForm.formState.errors.role && (
                    <p className="text-sm text-red-500">{promoteForm.formState.errors.role.message}</p>
                  )}
                  {isSuperAdmin && (
                    <p className="text-xs text-muted-foreground">
              ...
              <div className="flex gap-2">
                ...
                >
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : isAdmin ? "Update Role" : "Promote"}
                </Button>
                <Button type="button" variant="ghost" size="sm" onClick={closeForm} disabled={loading}>
                  Cancel
                </Button>
        ...
        {active === "delete" && (
          <div className="rounded-md border border-red-400 bg-red-50 dark:bg-red-950/20 p-5 space-y-4">
            ...
            <p className="text-sm text-red-700 dark:text-red-300">
              Warning: This action <strong>cannot be undone</strong>. User ID <strong>#{userId}</strong> and all associated data will be permanently removed.
            </p>
            ...
            {result && (
              <div className={`flex items-center gap-2 text-sm font-medium ${result === "success" ? "text-green-700" : "text-red-700"}`}>
                {result === "success" ? <CheckCircle className="h-5 w-5" /> : <XCircle className="h-5 w-5" />}
                {result === "success" ? "User deleted permanently" : "Delete failed"}
              </div>
            )}
          </div>
        )}
